---
title: "Controllers, Services, and the Generic CRUD Pattern"
sidebarTitle: "Controllers"
description: "The Ferreandina backend uses a generic Controller<T> and Service<T> pattern. Every resource gets full CRUD with zero boilerplate, plus hooks for custom queries."
---
...
<CardGroup cols={3}>
  <Card title="ResultUtil&lt;T&gt;" icon="envelope">
    A uniform JSON response wrapper. All responses share the same envelope shape — `message`, `status`, `data` (array), and `size`. Dedicated overloads handle `FindIterable<T>`, single documents, plain strings, and exceptions. On error, `javalinReturn(ctx, Exception e)` sets HTTP status 500 and returns a structured error object with `message`, `cause`, `class`, and `stack-trace`.
  </Card>
